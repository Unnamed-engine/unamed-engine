...
void Hush::VulkanRenderer::UpdateSceneObjects()
{
	...
	// camera projection
	this->m_sceneData.proj = glm::perspective(glm::radians(70.f), (float)this->m_width / (float)this->m_height, 0.1f, 10000.f);
...
void Hush::VulkanRenderer::InitRendering()
{
    ...
    //Initialize our editor camera
    this->m_mainCamera = EditorCamera(70.0f, static_cast<float>(this->m_width), static_cast<float>(this->m_height), 0.1f, 1000.0f);
}
